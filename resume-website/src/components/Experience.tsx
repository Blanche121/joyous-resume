        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            {exp.organizationLink ? (
              ...
              <h3 className="font-bold text-lg text-slate-800">{exp.organization}</h3>
            )}
            <p className="text-slate-500 text-sm mt-0.5">{exp.title}</p>
          </div>
        </div>
        <span className={`px-3 py-1 ${typeColors.bg} ${typeColors.text} rounded-full text-xs font-medium`}>
          {exp.type === "work" ? "实习" : exp.type === "research" && exp.organization.includes("MCM/ICM") ? "竞赛" : exp.type === "research" ? "研究" : "个人"}
        </span>
      </div>
        ...
        {exp.highlights.map((highlight, i) => {
          ...
          return (
            <li key={i} className="flex items-start gap-2 text-slate-600 text-sm">
              ...
              <span>
                <span className="font-bold text-slate-800">{prefix}</span>
                {suffix}
              </span>
            </li>
          );
